...
class DataGenerator:
    ...
    def parse_csv(self,
        ...
        csvfile = open(self.labels_filename, newline='')
        ...
        for row in csvread: # For every line (i.e for every bounding box) in the CSV file...
          ...
          name = row[self.input_format.index('image_name')]
          box.append(name.strip()) # Select the image name column in the input format and append its content to `box`
          ...
          for element in self.labels_output_format: # For each element in the output format (where the elements are the class ID and the four box coordinates)...
            if element == 'class_id':
              ...
              box.append(poly['x'] + poly['width'])
            elif element == 'ymax':
              box.append(poly['y'] + poly['height'])
          data.append(box)
